...
# V10: V7 stable base + Gemma preferred only for language categories.
# Keep safe deterministic solvers and conservative runtime; do NOT use risky consensus/review by default.
...
CATEGORY_MODEL_HINTS = {
    ...
    # Gemma is used selectively for language tasks only. It is never called unless the
    # harness includes it in ALLOWED_MODELS, so this still obeys model restrictions.
...
def score_model(model_id: str, category: str) -> int:
    ...
    if category in {"math", "logic"} and any(h in text for h in ("r1", "qwq", "reason")):
        score += 600
    if category in {"factual", "summary", "ner", "sentiment"} and "gemma" in text:
        score += 900
    return score
