...
async def run(tasks: list[dict[str, Any]], results: dict[str, str]) -> None:
    ...
    log("MODEL_PLAN", **MODEL_PLAN, version="V17.3", gemma_used=False)
...
def write_usage_log() -> None:
    try:
        ...
        with open(path, "w", encoding="utf-8") as file:
            json.dump({"version": "V17.3", "model_plan": MODEL_PLAN, "calls": CALL_LOG, "totals": totals}, file, ensure_ascii=False, indent=2)
    except Exception as error:
